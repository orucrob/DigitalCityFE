...
/***/ function(module, exports, __webpack_require__) {
	...
	function prepareData(json) {
		var data = d3.nest().key(function (d) {
			return getMonth(d["DatumZverejnenia"]);
	...
	function drawChart(chartData) {
		...
		var appWidth = getAppWidth();
		var width = appWidth - margin.left - margin.right;
		var height = getWindowHeight() / 2 - margin.top - margin.bottom;
		...
		//setup chart
		var chart = selectRoot('svg', 'chart').attr('width', width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom);
	...
	function openGrid(data) {
		...
		//PIE
		var pieSvg = selectRoot('svg', 'pie1');
	...
	function openGrid2(data) {
		...
		newRow.append("div").attr("class", "sum").text(function (d, i) {
			return curr(d["SumaCelkom"]);
		});
		newRow.append("div").attr("class", "date").text(function (d, i) {
			return df(d["DatumZverejnenia"]);
		});
		newRow.append("div").attr("class", "subject").text(function (d, i) {
			return d["Predmet"];
		});
