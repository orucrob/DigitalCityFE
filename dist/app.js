...
/***/ function(module, exports, __webpack_require__) {
	...
	var _d = __webpack_require__(301);

	var d3 = _interopRequireWildcard(_d);

	var _dataapi = __webpack_require__(302);

	var _dataapi2 = _interopRequireDefault(_dataapi);

	var _util = __webpack_require__(303);

	var _obec = __webpack_require__(304);

	var obec = _interopRequireWildcard(_obec);

	var _yearview = __webpack_require__(305);

	var yw = _interopRequireWildcard(_yearview);

	var _error = __webpack_require__(306);

	var err = _interopRequireWildcard(_error);
	...
	function init() {
		(0, _util.on)('dataapierror', function (status, xhr) {
			(0, _util.removeAll)();
			err.drawError(status, xhr);
		});
		(0, _util.on)('obecselect', function (otag) {
			obec.removeAll();
			yw.drawApp(otag);
		});
		yw.drawApp();
...
/***/ function(module, exports) {
	...
	var dataapi = {
		faDod: function faDod(oid, year) {
			return 'https://itinn.eu/dcapi/fakturyDodavatelske/' + oid + '/' + year + '?format=json';
		},
		faDodYears: function faDodYears(oid) {
			return 'https://itinn.eu/dcapi/GetRok/FDOD/' + oid + '?format=json';
		},
		organizacie: function organizacie() {
			return 'https://itinn.eu/dcapi/organizacie?format=json';
		}
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.getAppWidth = getAppWidth;
	exports.getWindowHeight = getWindowHeight;
	exports.getHash = getHash;
	exports.saveToHash = saveToHash;
	exports.on = on;
	exports.fire = fire;
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.getOrgRec = exports.getData = exports.draw = undefined;
	...
	var getOrgRec = exports.getOrgRec = function () {
		var _ref3 = _asyncToGenerator(regeneratorRuntime.mark(function _callee3(oTag) {
			var d, ret, i;
	...
	function loadData() {
		return new Promise(function (resolve, reject) {
			d3.json(_dataapi2.default.organizacie(), function (json) {
				if (!json) {
				...
				var xhr = ev.srcElement,
				    status = xhr.status;
				(0, _util.fire)('dataapierror', [status, xhr]);
			});
		});
	}
	function drawSerchBox(data) {
		data = data || [];
		var searchBox = (0, _util.selectRoot)('div', 'obecsearch');
	...
	function drawTable(data) {
		...
		data.sort(function (a, b) {
			return d3.ascending(a["Nazov"], b["Nazov"]);
		});

		var grid = (0, _util.selectRoot)('div', 'obecgrid');
		grid.style('border', '1px solid black');

		var row = grid.selectAll('.row').data(data).attr('data-id', function (d, i) {
			return d["OrganizaciaId"];
		});
		row.select(".id").text(function (d, i) {
			return d["OrganizaciaId"];
		});
		row.select(".hashtag").text(function (d, i) {
			return d["HashTag"];
		});
		row.select(".title").text(function (d, i) {
			return d["Nazov"];
		});
		row.select(".titleObec").text(function (d, i) {
			return d["NazovObec"];
		});

		row.exit().remove();

		var newRow = row.enter().append('div').attr('class', function (d, i) {
			return "row idx" + i;
		});
		newRow.append("div").attr("class", "id").text(function (d, i) {
			return d["OrganizaciaId"];
		});
		newRow.append("div").attr("class", "hashtag").text(function (d, i) {
			return d["HashTag"];
		});
		newRow.append("div").attr("class", "title").text(function (d, i) {
			return d["Nazov"];
		});
		newRow.append("div").attr("class", "titleObec").text(function (d, i) {
			return d["NazovObec"];
		});

		newRow.on('click', function (d, i) {
			(0, _util.saveToHash)('o', d["HashTag"]);
			(0, _util.fire)('obecselect', [d["HashTag"], d]);
		});
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	var drawApp = exports.drawApp = function () {
		var _ref = _asyncToGenerator(regeneratorRuntime.mark(function _callee(otag, year) {
			var orgRec, orgId;
			return regeneratorRuntime.wrap(function _callee$(_context) {
				while (1) {
					switch (_context.prev = _context.next) {
						case 0:
							otag = otag || (0, _util.getHash)()['o'];
							year = year || (0, _util.getHash)()['y'] || new Date().getFullYear();

							_context.next = 4;
							return obec.getOrgRec(otag);

						case 4:
							orgRec = _context.sent;
							orgId = orgRec && orgRec['OrganizaciaId'];


							if (orgId) {
								d3.json(_dataapi2.default.faDod(orgId, year), function (json) {
									if (json) {
										(function () {
											var chartData = prepareData(json);
		...
		return function drawApp(_x, _x2) {
			return _ref.apply(this, arguments);
		};
	}();
	...
	function prepareData(json) {
		var data = d3.nest().key(function (d) {
			return (0, _util.mf)(d["DatumZverejnenia"]);
		}).sortKeys(d3.ascending).rollup(function (dd) {
			return {
	...
	function drawChart(chartData, obecRec, year) {
		...
		var appWidth = (0, _util.getAppWidth)();
		var width = appWidth - margin.left - margin.right;
		var height = (0, _util.getWindowHeight)() / 2 - margin.top - margin.bottom;
		var barWidth = width / data.length;

		var x = d3.scalePoint().domain(keys).rangeRound([0, width]).padding(.5);
		var y = d3.scaleLinear().domain([0, d3.max(data, function (g) {
			return g.value.sum;
		})]).range([40, height]);
		...
		//setup title
		var chartTitle = (0, _util.selectRoot)('div', 'chartTitle');
		...
		d3.json(_dataapi2.default.faDodYears(obecRec["OrganizaciaId"]), function (json) {
			if (json && json.Data) {
				console.log('json', json, yearSel);
				var opt = yearSel.selectAll('option').data(json.Data);
				opt.enter().append('option').merge(opt).attr("id", function (d) {
					return d["id"];
				}).text(function (d) {
					return d["rok"];
				}).attr('selected', function (d) {
					return year == d["id"] ? true : null;
				});
				opt.exit().remove();
			}
		}).on('error', function (ev) {
		...
		//setup chart
		var chart = (0, _util.selectRoot)('svg', 'chart').attr('width', width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom);
		...
		//update
		var bar = chart.selectAll('g.bar').data(data).attr("transform", function (d, i) {
			return "translate(" + i * barWidth + ",0)";
		});
		var rect = bar.select('rect').attr("x", 3).attr("y", function (d) {
			return height - y(d.value.sum);
		}).attr("height", function (d) {
			return y(d.value.sum);
		}).attr("width", barWidth - 3);

		bar.select("text.label1").attr("x", barWidth / 2).attr("y", function (d) {
			return height - y(d.value.sum) + 3;
		}).attr("dy", ".75em").html(function (d) {
			return d.value.sum.toFixed(2) + " &euro;";
		});
		bar.select("text.label2").attr("x", barWidth / 2).attr("y", function (d) {
			return height - y(d.value.sum) + 20;
		}).attr("dy", ".75em").html(function (d) {
			return "#" + d.value.count;
		});

		//create
		var newBar = bar.enter().append('g').attr('class', 'bar').attr("transform", function (d, i) {
			return "translate(" + i * barWidth + ",0)";
		}).on('click', openGrid);

		var newRect = newBar.append("rect").attr("x", 3).attr("y", function (d) {
			return height - y(d.value.sum);
		}).attr("height", function (d) {
			return y(d.value.sum);
		}).attr("width", barWidth - 3);

		newBar.append("text").attr('class', 'label1').attr("x", barWidth / 2).attr("y", function (d) {
			return height - y(d.value.sum) + 3;
		}).attr("dy", ".75em").html(function (d) {
			return (0, _util.curr)(d.value.sum);
		});
		newBar.append("text").attr('class', 'label2').attr("x", barWidth / 2).attr("y", function (d) {
			return height - y(d.value.sum) + 20;
		}).attr("dy", ".75em").html(function (d) {
			return "#" + d.value.count;
		});
	...
	function openGrid(data) {
		...
		var gridData = d3.nest().key(function (d) {
			return d["DodavatelIco"];
		})
		...
		}).entries(data.value.detail).sort(function (a, b) {
			return d3.descending(a.value.sum, b.value.sum);
		});

		//PIE
		var pieSvg = (0, _util.selectRoot)('svg', 'pie1');
		pieSvg = pieSvg.attr('width', size).attr('height', size);
		var pieSvgG = pieSvg.select("g.ch");
		var pieSvgHover = pieSvg.select("g.hov");
		...
		var path = pieSvg.selectAll('path').data(pieData).attr('class', function (d, i) {
			return "p" + i;
		});
		...
		path.exit().remove();
		var newPath = path.enter().append('path').attr('d', arc).attr('fill', function (d, i) {
			return color(i);
		...
		.on('mouseover', pieOn).on('mouseout', pieOff);

		//selection for pie charts
		var pathHov = pieSvgHover.selectAll('path').data(pieData).attr('class', function (d, i) {
			return "p" + i;
		}).attr('d', arcOn);
		pathHov.exit().remove();
		var newPathHov = pathHov.enter().append('path').attr('d', arcOn).attr('fill', function (d, i) {
			var c = d3.color(color(i));
			c.opacity = 0.5;
			return c.toString();
		}).attr('class', function (d, i) {
			return "p" + i;
		...
		//GRID
		var grid = (0, _util.selectRoot)('div', 'grid1');

		grid.style('border', '1px solid black');
		var row = grid.selectAll('.row').data(gridData).attr('class', function (d, i) {
			return "row idx" + i;
		});
		row.select(".col").attr('min-width', '1em').style('min-height', '1em').style('background', function (d, i) {
			return color(i);
		});
		row.select(".sum").text(function (d, i) {
			return (0, _util.curr)(d.value.sum);
		});
		row.select(".title").text(function (d, i) {
			return d.value.name;
		});
		row.select(".subject").text(function (d, i) {
			return "#" + d.value.count;
		});
		...
		newRow.append("div").attr("class", "col").style('min-width', '1em').style('min-height', '1em').style('background', function (d, i) {
			return color(i);
		});
		newRow.append("div").attr("class", "sum").text(function (d, i) {
			return (0, _util.curr)(d.value.sum);
		});
		newRow.append("div").attr("class", "title").text(function (d, i) {
			return d.value.name;
		});
		newRow.append("div").attr("class", "subject").text(function (d, i) {
			return "#" + d.value.count;
		});
	...
	function openGrid2(data) {
		...
		var grid = (0, _util.selectRoot)('div', 'grid2');
		...
		row.select(".time").text(function (d, i) {
			return d["DatumZverejnenia"];
		});
		row.select(".subject").text(function (d, i) {
			return d["Predmet"];
		});
		...
		newRow.append("div").attr("class", "no").text(function (d, i) {
			return "#" + (i + 1);
		});
		newRow.append("div").attr("class", "sum").text(function (d, i) {
			return (0, _util.curr)(d["SumaCelkom"]);
		});
		newRow.append("div").attr("class", "date").text(function (d, i) {
			return (0, _util.df)(d["DatumZverejnenia"]);
		});
		newRow.append("div").attr("class", "subject").text(function (d, i) {
			return d["Predmet"];
		});
...
/***/ function(module, exports, __webpack_require__) {
	...
	function drawError(status, xhr) {
		var err = (0, _util.selectRoot)('div', 'error');
